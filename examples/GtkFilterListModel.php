 ...
 *
 *   bin/php-gtk4 examples/GtkFilterListModel.php
 */

require __DIR__ . '/bootstrap.php';

Demo::run('GtkFilterListModel', function (GtkWindow $win): GtkWidget {
    $everyone = Demo::store();
    $nobody = Demo::store();
    $nobody->remove_all();

    $modern = new GtkCustomFilter(static fn($item): bool => Demo::born($item) >= 1900);
    $filtered = new GtkFilterListModel($everyone, $modern);

    $note = 'model=5 people · filter=born >= 1900';
    $canvas = Demo::bars($filtered, function () use (&$note, $filtered, $everyone): string {
