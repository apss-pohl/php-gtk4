 ...
 *
 *   bin/php-gtk4 examples/GdkTexture.php
 */

require __DIR__ . '/bootstrap.php';

Demo::run('GdkTexture', function (GtkWindow $win): GtkWidget {
    $size = 8;
    ...
    return Demo::canvas(520, 300, static function (
        ...
        int $width,
        int $height,
    ) use ($size, $texture, $reloaded, $png, $path, $failure): void {
        Demo::sheet($cr);
        ...
        Demo::text($cr, $x, 98, sprintf('save_to_png_bytes() %d bytes', strlen($texture->save_to_png_bytes())), Demo::INK, 12);

        Demo::text($cr, $x, 132, 'save_to_png() + new_from_filename()', Demo::MUTED, 12);
        Demo::text($cr, $x, 150, sprintf('%d x %d px', $reloaded->get_width(), $reloaded->get_height()), Demo::INK, 13);
