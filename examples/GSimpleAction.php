 ...
 *
 *   bin/php-gtk4 examples/GSimpleAction.php
 */

require __DIR__ . '/bootstrap.php';

Demo::run('GSimpleAction', function (GtkWindow $win, GtkApplication $app): GtkWidget {
    /** @var list<string> $log */
    $log = [];
    $label = Demo::label();
    ...
    $ping->connect('activate', function (GSimpleAction $self) use (&$log, $render): void {
        $log[] = sprintf('activate %-7s parameter_type=%s', $self->get_name(), var_export($self->get_parameter_type(), true));
        $render();
    });
