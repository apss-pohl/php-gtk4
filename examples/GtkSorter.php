 ...
 *
 *   bin/php-gtk4 examples/demo.php GtkSorter
 */
...
return Demo::page(
    'GtkSorter',
    'the abstract base: one method, changed()',
    function (GtkWindow $win): GtkWidget {
        $store = Demo::store();

        // Shared mutable state, read by the comparator that is written before the
        // handler flipping it.
        $state = new class {
        ...
        $canvas = Demo::bars($sorted, function () use (&$note, $state): string {
            return sprintf("%s\n%s", $state->descending ? 'descending' : 'ascending', $note);
        });
        ...
        $step = 0;
        $button->connect('clicked', function () use ($sorter, $state, &$step, &$note, $canvas): void {
            if ($step++ % 2 === 0) {
        ...
        Demo::status(sprintf('abstract base of %s', $sorter::class));
        return $button;
    },
    480,
    320,
