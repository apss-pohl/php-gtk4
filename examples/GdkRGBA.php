 ...
 *
 *   bin/php-gtk4 examples/GdkRGBA.php
 */

require __DIR__ . '/bootstrap.php';

Demo::run('GdkRGBA', function (GtkWindow $win): GtkWidget {
    ...
    return Demo::canvas(520, 260, static function (
        ...
        Demo::text($cr, 18, 200, sprintf('$a->equal($b)          %s', $a->equal($b) ? 'true' : 'false'), Demo::INK, 12);
        Demo::text($cr, 18, 218, sprintf('$c = clone $a; alpha=.5 -> equal  %s', $a->equal($c) ? 'true' : 'false'), Demo::INK, 12);
        Demo::text(
            $cr,
            18,
            236,
        ...
        $probe = new GdkRGBA();
        Demo::text($cr, 18, 254, sprintf('(new GdkRGBA)->parse("nope") -> %s', $probe->parse('nope') ? 'true' : 'false'), Demo::MUTED, 12);
    });
}, 540, 300);
