 ...
 *
 *   bin/php-gtk4 examples/GtkSortListModel.php
 */

require __DIR__ . '/bootstrap.php';

Demo::run('GtkSortListModel', function (GtkWindow $win): GtkWidget {
    $store = Demo::store();
    $filter = new GtkCustomFilter(static fn($item): bool => Demo::born($item) >= 1900);
    $filtered = new GtkFilterListModel($store, $filter);

    $sorter = new GtkCustomSorter(static fn($a, $b): int => Demo::born($b) <=> Demo::born($a));
