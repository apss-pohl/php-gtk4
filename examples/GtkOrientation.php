 ...
 * couple of seconds.
 *
 *   bin/php-gtk4 examples/demo.php GtkOrientation
 */
...
return Demo::page(
    'GtkOrientation',
    'Horizontal or Vertical, the two-case GEnum',
    function (GtkWindow $win): GtkWidget {
        // Shared mutable state, read by the draw func written before the timer.
        $state = new class {
            public GtkOrientation $current = GtkOrientation::Horizontal;
        };
        ...
        $area = Demo::canvas(420, 260, static function (
            ...
            foreach ($lengths as $index => $length) {
                ...
                if ($state->current === GtkOrientation::Horizontal) {
            ...
            Demo::text(
                ...
                24,
                34,
                sprintf('GtkOrientation::%s (value %d)', $state->current->name, $state->current->value),
                Demo::INK,
                16,
            );
            Demo::text(
        ...
        GLib::timeout_add(1800, function () use ($area, $state): bool {
            $state->current = $state->current === GtkOrientation::Horizontal
