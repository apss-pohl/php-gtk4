...
src/<Ns>/           one .cpp per class: ZEND_METHODs, generated by gen/gir.php (GENERATED header) or hand-written
  │
...
- `struct Object { GObject *obj; bool held; zend_object std; }`; `create_object` allocates it for every
  registered class (one struct serves all — per-class state lives in the GObject).
- Handlers: `free_obj` (clear qdata, `g_object_remove_toggle_ref`), `clone_obj = NULL`,
...
- **Not covered (yet)**: declaring GObject properties or signals in PHP, vfuncs whose arguments
  are pointers to scalars without a GIR direction (`compute_expand`). `snapshot()` is covered
  since `GtkSnapshot` landed (milestone 4, 2026-09-02): `GtkWidget::vfunc_snapshot()`.
  `GObject`'s own vfuncs (`dispose`, `set_property`, …) are hand-written territory and not exposed.
...
- it validates the generator on a realistic subset before it touches 500 classes (wave 0
  regenerated the then hand-written classes; the suite was adapted to GTK's own API shape);
...
1. **Allow-list + transitive closure.** `gen/allowlist.txt` names the classes/enums of the wave.
   The generator adds what they need: parent classes, implemented interfaces (and an interface's
   interface prerequisites), and the enums/flags used in kept signatures. Types that appear in a
   signature but are not in the closure are *not* pulled in: the member is skipped and listed in
   `gen/report.md` (widen the allow-list to get it), never
...
2. **Generated files are owned by the generator.** They carry a `// GENERATED by gen/gir.php -
   ...
   differs. "Hand-writing what's needed" happens in exactly four places:
   ...
   - `src/classes.h` + `src/gtk4.cpp` MINIT — a non-GObject type (boxed, fundamental, own object
     layout) is registered by hand there; the generator only emits `gen_minit.inc` for GObject
...
- List widgets: `GtkListView`/`GtkColumnView` + `GListModel`/`GtkStringList`/`GtkSelectionModel`;
  ...
  for the `PhpCallable` design. `GtkTreeView` still exists but is deprecated since 4.10, and the
  generator skips every deprecated member (`methodSkipReason()` in `gen/gir.php`) — it stays
  unbound; the list widgets are its replacement.
...
php-gtk4/
  ...
  config.w32          the same build for the PHP SDK on Windows (MSVC, GTK from gvsbuild)
  src/gtk4.cpp        module entry, MINIT (hand-written classes, then #include "gen_minit.inc"), RINIT/RSHUTDOWN
  src/gtk4.stub.php   the hand-written API declaration -> src/gtk4_arginfo.h (generated, committed)
  src/classes.h       registration hooks of the non-GObject hand-written classes (boxed, fundamental, ...)
  src/core/           hand-written runtime (§2)
  src/<Ns>/           one .cpp per class, generated in place (GENERATED header) or hand-written, plus the
                      generated per-namespace <Ns>.stub.php -> <Ns>_arginfo.h (enum-only namespaces such as
                      Pango have just those two)
  src/gen_minit.inc   generated MINIT block; gen_prototypes.h, gen_arginfo.h its companions
  gen/                gir.php (the generator) + inputs allowlist/handwritten/skip/ctor-props/smoke-skip.txt
                      and overrides/; map-status.php; gen_stub.php (vendored), ide-stub.php,
                      method-comments.php; report.md (what was skipped and why); README.md (the flow)
  stubs/gtk4.php      generated IDE stub (hand-written + every per-namespace stub)
  tests/              PHPUnit 12 suite (+ Generated/ smoke tests, Subclass/ fixtures), tests/phpt
                      (run-tests.php), tests/scripts (stress, shutdown)
  examples/           demo.php + bootstrap.php + one <Class>.php page per registered class
  docs/               PLAN, TODO, GTK3-MAP (status column generated), BUILD, INSTALL, CONTRIBUTING, RELEASING
...
- Minimum suites: object identity & refcount (`===`, the toggle-ref hold across GTK ownership), signal
  marshalling of
