...
([scorninpc/php-gtk3](https://github.com/scorninpc/php-gtk3), `src/`), its GTK 4 replacement, and whether
php-gtk4 already implements it.
...
| 🧩 | out of scope for now (separate library / later milestone: GtkSourceView, Glade, …) |
...
| ✅ implemented | 87 | — |
| 🟡 partial | 3 | — |
| ❌ to port (GTK 4 equivalent exists) | 6 | ~1750 |
| ⛔ removed in GTK 4 | 41 | ~520 |
| 🧩 out of scope / later milestone | 3 | ~130 |

php-gtk4 currently declares (2026-09-10) 525 classes, interfaces and enums: 28 hand-written
(`src/gtk4.stub.php`) and 497 generated - Gtk (213), WebKit (119), Gsk (50), Gdk (32), Gio (32), Pango (22),
...
| `Gtk::show_uri_on_window` | `GtkUriLauncher` (4.10+) | ✅ | `gtk_show_uri` is deprecated in 4.10; the launcher answers in a callback. |
...
| php-gtk3 class | gtk3 methods | GTK 4 replacement | php-gtk4 | Notes |
| --- | ---: | --- | :---: | --- |
| `GtkWidget` | 257 | `GtkWidget` | ✅ | Generated (wave 0): the whole GTK 4 method set (layout, margins, align/expand, focus, tree navigation, state flags, tooltips, CSS classes, actions). The ~230 remaining php-gtk3 methods are GTK 3-only (`GdkEvent` unions, `size_allocate` w/ `GtkAllocation`, style properties, `gtk_widget_destroy`, DnD, accel closures) or need types outside the closure (`GtkLayoutManager`, `GdkDisplay`, controllers — later waves). |
...
| php-gtk3 class | gtk3 methods | GTK 4 replacement | php-gtk4 | Notes |
| --- | ---: | --- | :---: | --- |
| `GtkWindow` | 85 | `GtkWindow` | ✅ | Generated (wave 0): the GTK 4 method set (title, sizes, modal/resizable/decorated/deletable, fullscreen/maximize, transient-for, default widget, focus, hide-on-close, …). **Removed in GTK 4:** `move`/`resize`/`get_position`/`get_size`, `set_position`, `set_type_hint`, `iconify`/`stick`/`set_keep_above`, `set_icon*`, `get_screen`, `add_accel_group`. |
| `GtkApplicationWindow` | 2 | `GtkApplicationWindow` | ✅ | Takes the application in its constructor and holds the window-scoped action map; `examples/notes/` subclasses it. |
| `GtkButton` | 29 | `GtkButton` | ✅ | Generated (wave 0): the GTK 4 method set (label, child, icon name, has-frame, use-underline, static `new_with_label`/`new_with_mnemonic`/`new_from_icon_name`). **Removed:** `set_image`/`set_always_show_image`/`set_relief`/`set_alignment`/`set_use_stock`/`get_event_window`, `enter`/`leave`/`pressed`/`released` signals. |
...
| `GtkColorButton` | 13 | `GtkColorDialogButton` (4.10+) | ✅ | The button owns a `GtkColorDialog` and answers with its `rgba` property; `GtkColorButton` itself is deprecated and stays unbound. |
| `GtkFontButton` | 16 | `GtkFontDialogButton` (4.10+) | ✅ | As the colour button, with a `PangoFontDescription` in `font-desc` and `GtkFontLevel` deciding how much of a font the dialog offers. |
...
| `GtkLabel` | 43 | `GtkLabel` | ✅ | Generated (wave 0): the GTK 4 method set (text/markup/mnemonic, `set_use_markup`, ellipsize, justify, wrap (`set_line_wrap` renamed), lines, width chars, xalign/yalign, mnemonic widget, selection, `get_current_uri`). `set_angle`/`set_pattern`/`get_layout` are GTK 3-only or Pango-dependent. |
...
## Pango (`php-gtk3/src/Pango/`)

| php-gtk3 class | gtk3 methods | GTK 4 replacement | php-gtk4 | Notes |
| --- | ---: | --- | :---: | --- |
| `PangoLayout`, `PangoContext`, `PangoAttrList`, `PangoLayoutLine`, `PangoWrapMode` | 18 | `PangoLayout` / `PangoContext` / `PangoAttrList` / `PangoLayoutLine` / `PangoWrapMode` | 🟡 | Pango is unchanged between GTK 3 and 4 and bound with the drawing cluster. `PangoLayoutLine` is the one leaf still missing — it costs `PangoLayout::get_line*()`/`get_lines*()` (`gen/report.md`). |
