...
edited; `src/gtk4.cpp` only `#include`s `gen_minit.inc` after the hand-written registrations
(plus `gen_minit_defs.inc` at file scope, which holds one `register_<feature>_classes()` per
conditional namespace so MINIT does not grow a whole feature's registrations), and
