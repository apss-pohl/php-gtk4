...
final class Generator
{
    ...
    public function run(): void
    {
            ...
            foreach ($nodes as $n) {
                switch ($n->kind) {
                    ...
                    case 'class':
                        ...
                        break;
                    default:
                        $this->skip($n, '*', "{$n->kind}: not generated (records/boxed are hand-written for now)");
                }
            }
        ...
        file_put_contents($reportPath, rtrim($report) . "\n");
    }
    ...
    private function writeSmokeTests(): void
    {
        ...
        foreach (glob("$dir/*SmokeTest.php") ?: [] as $stale) {
            unlink($stale);  // the set is owned by the generator
        }
        foreach ($this->classInfo as $php => ['node' => $n, 'stub' => $stub]) {
            ...
            file_put_contents("$dir/{$php}SmokeTest.php", $test);
    ...
    private function writeExamples(): void
    {
        ...
        foreach (array_keys($this->emit) as $q) {
            $n = $this->gir->types[$q];
            if (!in_array($n->kind, ['class', 'interface', 'enum', 'bitfield'], true)) {
                continue;
            }
            $php = phpClass($n);
            if (!isset($known[$php])) {
        ...
        file_put_contents($sections, "<?php\n\n"
    ...
    private function examplePage(Node $n): string
    {
        ...
        return $head . "use Gtk4\\GtkLabel;\nuse Gtk4\\GtkWidget;\nuse Gtk4\\GtkWindow;\n\n"
            ...
            . " * shows $php doing something and move the class out of the 'Generated' section.\n *\n"
            . " *   bin/php-gtk4 examples/demo.php $php\n */\n\n"
            . "require_once __DIR__ . '/bootstrap.php';\n\n"
            . "return Demo::page(\n    '$php',\n    '$summary',\n    function (GtkWindow \$win): GtkWidget {\n"
            . "        \$label = new GtkLabel();\n"
    ...
    private function write(string $rel, string $text): void
    {
        ...
        file_put_contents($path, rtrim($text) . "\n");
        if (str_ends_with($rel, '.cpp') && self::clangFormat() !== null) {
            // Same style the cpp-lint stage enforces (.clang-format); the generator is not a formatter.
            shell_exec(escapeshellarg(self::clangFormat()) . ' -i --style=file ' . escapeshellarg($path));
        }
        $this->files[$rel] = $path;
    }
    ...
    private function emitClass(Node $n, array &$minit, array &$protos): array
    {
        ...
        if ($n->kind === 'class' && !$n->final && $n->vfuncs !== []) {
        ...
        foreach (
            ['enum_' => '"core/enums.h"', 'strv_' => '"core/collections.h"', 'glist_to_php' => '"core/collections.h"',
                ...
                'wrap_cairo' => '"core/cairo.h"', 'wrap_param_spec' => '"core/paramspec.h"',
                'subtype_' => '"core/subtype.h"', 'report_pending_exception' => '"core/error.h"',
                'std::array' => '<array>'] as $needle => $inc
        ) {
            if (
                str_contains($joined, $needle) && !in_array($inc, $includes, true)
    ...
    private function methodSkipReason(Node $n, Func $f): ?string
    {
        ...
        foreach ($f->params as $p) {
            ...
            if (in_array($p->type->name, ['gpointer', 'gconstpointer'], true) && $p->name !== 'user_data') {
    ...
    private function method(
        ...
        $required = count(array_filter($ins, fn($p) => $p['default'] === null));
        foreach ($ins as $p) {
            $lines[] = '  ' . $p['decl'];
        }
        if ($ins === []) {
            ...
            $lines[] = "  ZEND_PARSE_PARAMETERS_START($required, " . count($ins) . ')';
        ...
        $callArgs = [];
        if ($f->kind === 'method') {
            $lines[] = "  {$n->ctype} *self = PHPGTK_SELF({$n->ctype}, $typeMacro);";
    ...
    private function zvalToC(Func $v, array $outs): array
    {
        ...
        $outLines = function (string $arrayExpr) use ($outs): array {
            ...
            foreach ($outs as $i => $o) {
                $get = match ($o['add']) {
                    'add_next_index_long' => "static_cast<{$o['ctype']}>(zval_get_long(e$i))",
                    'add_next_index_bool' => "zend_is_true(e$i) ? TRUE : FALSE",
    ...
    /** PHP scalar type of an out parameter mapping. */
    private static function outScalar(array $o): string
    {
        return match ($o['add']) {
            'add_next_index_long' => 'int', 'add_next_index_bool' => 'bool', default => 'float'
        };
    }
    ...
    /** @return array{string, string, string}|null C type, initializer, add_next_index_* function */
    private function outCType(Type $t): ?array
    {
        return match (true) {
            $t->name === 'gboolean' => ['gboolean', 'FALSE', 'add_next_index_bool'],
            $t->name === 'gfloat' => ['float', '0', 'add_next_index_double'],
            $t->name === 'gdouble' => ['double', '0', 'add_next_index_double'],
            (bool) preg_match(INT_TYPES, $t->name) => [rtrim($t->ctype ?? 'int', '*'), '0', 'add_next_index_long'],
            default => null,
        };
    }
    ...
    private function retMapping(Node $n, Func $f, array $outs): string|array
    {
        ...
        $throwCheck = fn(string $cond) => $f->throws
            ...
            : [];
        $outLines = function () use ($outs) {
            $l = ['array_init_size(return_value, ' . count($outs) . ');'];
            foreach ($outs as $o) {
                $l[] = "{$o['add']}(return_value, {$o['name']});";
            }
            return $l;
        };
        ...
        if ($t->name === 'gboolean') {
            if (count($outs) === 1) {
                $o = $outs[0];
                $scalar = self::outScalar($o);
                $retval = match ($scalar) {
                    'int' => 'RETVAL_LONG', 'bool' => 'RETVAL_BOOL', default => 'RETVAL_DOUBLE'
                };
                return ['phpType' => "?$scalar", 'lines' => fn(string $call) => [
                    "if (!$call) RETURN_NULL();", "$retval({$o['name']});"]];
            }
            if ($outs !== []) {
        ...
        if (in_array($node->kind, ['class', 'interface'], true)) {
            ...
            return ['phpType' => ($f->retNullable ? '?' : '') . $phpT, 'lines' => fn(string $call) => [
                "{$node->ctype} *result = $call;", ...$throwCheck('result == nullptr'),
