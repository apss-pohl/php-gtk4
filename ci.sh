...
stage_cpp_lint() {
    ...
    # The WebKit namespaces (src/WebKit, src/JavaScriptCore, src/Soup - CONDITIONAL_NAMESPACES in
    # gen/gir/config.php) include webkit/webkit.h, so clang-tidy can only parse them where the
    # headers are installed (cpp-lint.yml installs them; locally they are optional). clang-format
    # still covers them: it reads no headers.
    local -a tidy_prune=()
    if ! pkg-config --exists webkitgtk-6.0 2>/dev/null; then
        tidy_prune=(-path src/WebKit -prune -o -path src/JavaScriptCore -prune -o -path src/Soup -prune -o)
        echo "  (no webkitgtk-6.0 headers: src/WebKit, src/JavaScriptCore and src/Soup are not analysed)"
    fi
    mapfile -t cpp_files < <(find src "${tidy_prune[@]}" -type f -name '*.cpp' -print | sort)
