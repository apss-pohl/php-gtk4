...
stage_cpp_lint() {
    ...
    # The WebKit namespaces (src/WebKit, src/JavaScriptCore - CONDITIONAL_NAMESPACES in
    ...
    if ! pkg-config --exists webkitgtk-6.0 2>/dev/null; then
        tidy_prune=(-path src/WebKit -prune -o -path src/JavaScriptCore -prune -o)
        echo "  (no webkitgtk-6.0 headers: src/WebKit and src/JavaScriptCore are not analysed)"
    fi
