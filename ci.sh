...
#
#   stubs      gen_stub.php (src/gtk4_arginfo.h) + gen/ide-stub.php (stubs/gtk4.php) are up to date
...
# ---------------------------------------------------------------- stubs
# stubs/gtk4.stub.php is the API source of truth. Both derived files are
# committed; this stage regenerates them and fails if they changed.
stage_stubs() {
    ensure_vendor
    step "stubs: gen_stub.php -> src/gtk4_arginfo.h, ide-stub.php -> stubs/gtk4.php"
    local before; before=$(md5sum src/gtk4_arginfo.h stubs/gtk4.php 2>/dev/null)
    "$PHP" gen/gen_stub.php stubs/gtk4.stub.php >/dev/null || fail "gen_stub.php"
    mv -f stubs/gtk4_arginfo.h src/gtk4_arginfo.h
...
build_variant() {  # build_variant <output.so> [configure args...]
    ...
    ./configure --with-php-config="$PHP_CONFIG" "$@" > .configure.log 2>&1 || { tail -20 .configure.log; fail "configure"; }
    make -j"$JOBS" > .make.log 2>&1 || { grep -E 'error|warning' .make.log | head -40; fail "make"; }
    if grep -q 'warning:' .make.log; then grep 'warning:' .make.log | head -20; fail "compiler warnings"; fi
